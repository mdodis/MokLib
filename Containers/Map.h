...
template <typename TKey, typename TVal> struct TMapIterator;

/**
 * A Simple Fixed-allocation Hashtable implementation, using Coalesced Chaining
 */
template <typename TKey, typename TValue> struct TMap {
  struct Bin {
    TKey key;
    TValue val;
    Bin *next;
  ...
  TMapIterator<TKey, TValue> end() const {
    return TMapIterator<TKey, TValue>(values, num_total_bins, num_total_bins);
  }
...
template <typename TKey, typename TVal> struct TMapPair {
  TKey &key;
  TVal &val;
};
