...
#define PROC_IMPORT_PNG_ERROR_HANDLER(name) \
    void name(png_structp pngptr, png_const_charp error_msg)
...
{
    Allocator* alloc = (Allocator*)png_get_mem_ptr(ptr);
...
{
    Allocator* alloc = (Allocator*)png_get_mem_ptr(png_ptr);
...
PROC_IMPORTER_LOAD(import_png_load)
{
    ...
    Arena temp(System_Allocator, MEGABYTES(1));

    u8 signature[PngSignatureSize];
    if (read_file(file_handle, signature, PngSignatureSize, 0) !=
        PngSignatureSize)
    {
        return false;
    }
    ...
    png_structp png_ptr = png_create_read_struct_2(
        ...
        (png_voidp)import_png_user_error,
        import_png_user_error,
        import_png_user_error,
        (png_voidp)&temp,
        import_png_malloc,
        import_png_free);
    ...
    if ((bits_per_channel == 8) && (num_channels == 3) &&
        (color_type == PNG_COLOR_TYPE_RGB))
    {
        result->image.format = PixelFormat::RGB8X;
    } else if (
        (bits_per_channel == 8) && (num_channels == 4) &&
    ...
    result->data.size = result->image.width() * result->image.height() *
                        ((bits_per_channel * num_channels) / 8);

    result->data.buffer = alloc->reserve(result->data.size);
    result->image.pitch =
        result->image.width() * ((bits_per_channel * num_channels) / 8);
    result->image.bpp = num_channels * bits_per_channel;

    for (i32 i = 0; i < result->image.height(); ++i) {
        row_ptrs[i] = umm(result->data.buffer) +
                      (result->image.height() - i - 1) * result->image.pitch;
    }
