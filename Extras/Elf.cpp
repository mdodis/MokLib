...
namespace Elf {
    ...
    Result<ReadContext, Str> open(Tape* tape, Allocator& allocator)
    {
        Header64* header;
        {
            ParseTape pt(tape);
        ...
        ReadContext result = {tape, header};
        ...
        if (!result.enumerate_sections(find_string_table, allocator)) {
            return Err(
                LIT("Enumerate sections failed. File is probably malformed"));
        }
    ...
    bool ReadContext::enumerate_sections(
        EnumerateSectionsDelegate& delegate, Allocator& allocator)
    {
        ParseTape pt(tape);
        DEFER(pt.restore());
    ...
    SecHeader64* ReadContext::get_section_header(Str name, Allocator& allocator)
    {
