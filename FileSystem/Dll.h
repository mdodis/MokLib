...
struct TNamedProc {
    ...
    TNamedProc(const char* name) : name(name) {}
...
#if OS_MSWINDOWS
    Str get_filename(Allocator& alloc, Win32::Handle _process = 0);
#elif OS_LINUX
    Str   get_filename(Allocator& alloc, int process = 0);
#else
...
MOKLIB_API bool get_current_process_dlls(Allocator& alloc, TArray<Dll>& result);
