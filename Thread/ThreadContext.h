...
struct MOKLIB_API ThreadContextBase {
    ...
    virtual IAllocator &get_temp_allocator() = 0;
...
struct MOKLIB_API SimpleThreadContext : ThreadContextBase {
    ...
    IAllocator &get_temp_allocator() override {
...
#define MAKE_TEMP_ARENA(name, size) CREATE_SCOPED_ARENA(&ThreadContextBase::get<ThreadContextBase>()->get_temp_allocator(), name, size)