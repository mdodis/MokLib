...
static bool do_next_job_entry(JobQueue* queue)
{
    ...
    u32 new_next_read_entry =
        (original_next_read_entry + 1) % queue->num_entries;
...
void JobQueue::init(Allocator& alloc, u32 max_entries)
{
    this->alloc = &alloc;
    num_entries = max_entries;
    entries =
        (JobQueue::Entry*)alloc.reserve(num_entries * sizeof(JobQueue::Entry));
