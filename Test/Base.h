...
struct MOKLIB_API TestRunner {
    TestRunner()
        : test_cases(&allocator), benchmarks(&allocator)
    {}

    void add_test(TestCase& test_case) { test_cases.add(test_case); }

    void add_benchmark(Benchmark& benchmark) { benchmarks.add(benchmark); }
