...
static _inline bool operator==(
    const InputLayoutAttr& a, const InputLayoutAttr& b)
{
    return (a.slot == b.slot) &&
           ((a.name == b.name) || ((a.name != 0) && (b.name != 0) &&
                                   (strcmp(a.name, b.name) == 0))) &&
           (a.kind == b.kind) && (a.offset == b.offset) &&
           (a.stride == b.stride) && (a.usage == b.usage) &&
...
#define PROC_RAI_CREATE_RENDER_PASS(name) RenderPass name(RenderPassDesc* desc)

#define PROC_RAI_GET_DEFAULT_TEXTURE(name) \
    TextureRes name(ETextureResUsage kind)
#define PROC_RAI_GET_DEFAULT_RENDER_PASS(name) RenderPass name(void)

#define PROC_RAI_SET_PIPELINE(name) bool name(const Pipeline* pipeline)
#define PROC_RAI_SET_BINDINGS(name) void name(const Bindings* bindings)
#define PROC_RAI_SET_VIEWPORT(name) void name(const Viewport* viewport)
#define PROC_RAI_SET_RENDER_PASS(name) \
    void name(const RenderPass* pass, Vec2i size)
#define PROC_RAI_DRAW(name) void name(i32 base, i32 num_elements)
...
#define PROC_RAI_MAP_BUFFER(name) void* name(BufferRes* buffer)
#define PROC_RAI_UNMAP_BUFFER(name) void name(BufferRes* buffer)
#define PROC_RAI_UPDATE_BUFFER(name) \
    void name(BufferRes* buffer, u32 offset, Raw new_data)
#define PROC_RAI_GET_BUFFER_SIZE(name) u32 name(BufferRes* buffer)
#define PROC_RAI_RESIZE_BUFFER(name) void name(BufferRes* buffer, u32 new_size)
...
#define PROC_RAI_INITIALIZE(name) bool name(void* vparams, RAIGraphics& result)
