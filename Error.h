...
struct TEnum {
    T e;
    TEnum(const T& e) : e(e) {}
    _inline operator bool() const { return e != (T)0; }
};
...
struct TErr : TEnum<T> {
    ...
    TErr(const T& e, Str extra = Str::NullStr) : TEnum<T>(e), extra(extra) {}
};
...
static _inline Str format_error(const IOError& err)
{
    switch (err) {
        ...
        case IOError::FileNotFound:
            return LIT("File not found");
        case IOError::UnresolvedModuleDependencies:
            return LIT("Module dependencies could not be resolved");
        default:
            return LIT("Unrecognized");
    }
}
